...
export class RecordService {
  ...
  async createRecord(dto) {
    ...
    try {
      ...
      try {await DiscordAdapter.sendRecordCreated({group, record: saved});}
      catch (e2) {
  ...
  async getRecord({groupId, orderBy, nickname, page, pageSize}) {
    return await this.#repos.recordRepo.findMany({groupId, orderBy, nickname, skip: (page - 1) * pageSize, take: pageSize});
  }
